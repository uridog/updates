...
socket_running = True
client11 = Client(s)
start_window = Ui_startWindow(client11)
leaderboard_window = Ui_MainWindow(client11)
tutorial_window = Ui_TutorialWindow(client11)
round_num = 0
...
while client11.screen_state == -1:
    pass
if client11.screen_state == 0:
    sign_in_window = Ui_signUpWindow(client11)
    try:
        e = th.Thread(target=show_sign_window, args=(sign_in_window,))
        e.start()
    except:
        pass
    while client11.username == "":
        pass
    userInfo = client11.username + " " + client11.first_name + " " + client11.last_name + " " + client11.password
    s.send(create_msg(userInfo, "01").encode())
    current_username = client11.username
    data_received, cmd = handle_data(s)
    while cmd == "20":
        client11.server_ans = True
        sign_in_window.username_taken.show()
        client11.server_ans = False
        while client11.username == current_username:
            pass
        userInfo = client11.username + " " + client11.first_name + " " + client11.last_name + " " + client11.password
        s.send(create_msg(userInfo, "01").encode())
        client11.server_ans = True
    client11.client_singed = True
    data_received, cmd = handle_data(s)
elif client11.screen_state == 1:
    log_in_window = Ui_LogInWindow(client11)
    try:
        e = th.Thread(target=show_window, args=(log_in_window,))
        e.start()
    except:
        pass
    while client11.username == "" and client11.password == "":
        pass
    userInfo = client11.username + " " + client11.password
    old_username = client11.username
    old_password = client11.password
    ...
    while cmd == "23" or cmd == "24" or cmd == "27":
        ...
        client11.server_ans = False
        while client11.username == old_username and client11.password == old_password:
            pass
        old_username = client11.username
        old_password = client11.password
        userInfo = client11.username + " " + client11.password
        ...
        client11.server_ans = True
    client11.client_singed = True
ready_window = Ui_readyWindow(client11)
time.sleep(0.3)
...
while socket_running:
    client11.client_ready = False
    round_num = 0
    client11.screen_state = 2
    client11.game_start = False
    leaderboard_list = []
    while client11.client_ready is False:
        if client11.screen_state == 4:
            for i in range(3):
            ...
            s.send(create_msg(client11.username, "07").encode())
        ...
        if client11.screen_state == 5:
            try:
                ...
                pass
            print("sf42e3")
            while client11.record is False:
                pass
            print("sf4uir")
            MyText = " "
    ...
    s.send(create_msg(client11.username, "03").encode())
    client11.screen_state = 3
    game_window = Ui_gameWindow(client11)
    try:
    ...
    while round_num < 3:
        ...
        while cmd != "10":
            MyText = ""
            if client11.voice is True:
                voice_to_text()
            else:
                while client11.curr_word == "":
                    pass
                print(client11.curr_word)
                MyText = client11.curr_word
